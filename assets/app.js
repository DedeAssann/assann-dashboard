...
const legacyStorageKey='assann-dashboard-habits-v1';
let history=JSON.parse(localStorage.getItem(storageKey)||localStorage.getItem(legacyStorageKey)||'{}');
...
function reloadHistory(){try{history=JSON.parse(localStorage.getItem(storageKey)||'{}')}catch{history={}};if(!history[todayKey])history[todayKey]={};habits.forEach(h=>{if(typeof history[todayKey][h]!=='boolean')history[todayKey][h]=false})}
...
async function loadEnsmaCalendar(){try{const response=await fetch(`data/calendar.json?v=${Date.now()}`,{cache:'no-store'});if(!response.ok)return;const data=await response.json();if(!Array.isArray(data.events)||!data.events.length)return;ensma=data.events.map(e=>({date:formatEventDate(e.start),name:e.title||'ENSMA',meta:formatEventMeta(e),start:e.start}))}catch(error){console.info('ENSMA live calendar unavailable; using fallback data.',error)}}
...
window.addEventListener('assann:habits-synced',()=>{reloadHistory();renderHabits();renderAnalytics()});
